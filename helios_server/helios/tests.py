...
from helios_server.helios_auth import models as auth_models
...
class ElectionModelTests(TestCase):
    ...
    def test_facebook_eligibility(self):
        self.election.eligibility = [
            {
                ...
                "constraint": [{"group": {"id": "123", "name": "Fake Group"}}],
            }
        ]
        ...
        from helios_server.helios_auth.auth_systems import facebook

        def fake_check_constraint(constraint, user):
            return (
                constraint == {"group": {"id": "123", "name": "Fake Group"}}
                and user == self.fb_user
            )
...
class LegacyElectionBlackboxTests(DataFormatBlackboxTests, TestCase):
    ...
    EXPECTED_ELECTION_FILE = (
        "helios_server/helios/fixtures/legacy-election-expected.json"
    )
    EXPECTED_ELECTION_METADATA_FILE = (
        "helios_server/helios/fixtures/legacy-election-metadata-expected.json"
    )
    EXPECTED_VOTERS_FILE = (
        "helios_server/helios/fixtures/legacy-election-voters-expected.json"
    )
    EXPECTED_TRUSTEES_FILE = (
        "helios_server/helios/fixtures/legacy-trustees-expected.json"
    )
    EXPECTED_BALLOTS_FILE = "helios_server/helios/fixtures/legacy-ballots-expected.json"
...
class WebTest(django_webtest.WebTest):
    def assertStatusCode(self, response, status_code):
        actual_code = (
            ...
            else response.status_int
        )
        if isinstance(status_code, (list, tuple)):
            assert actual_code in status_code, "%s instad of %s" % (
                actual_code,
                status_code,
            )
        else:
            assert actual_code == status_code, "%s instad of %s" % (
                actual_code,
                status_code,
            )
...
class ElectionBlackboxTests(WebTest):
    ...
    def test_election_params(self):
        ...
        self.assertEqual(
            response.content, views.ELGAMAL_PARAMS_LD_OBJECT.serialize().encode("utf-8")
        )
    ...
    def test_get_election_stats(self):
        self.setup_login(
            from_scratch=True, user_id="mccio@github.com", user_type="google"
        )
    ...
    def _setup_complete_election(self, election_params=None):
        ...
        FILE = "helios_server/helios/fixtures/voter-badfile.csv"
    ...
    def _cast_ballot(
        ...
        ballot = datatypes.LDObject.fromDict(
            the_ballot, type_hint="legacy/EncryptedVote"
        )
    ...
    def test_do_complete_election_private(self):
        ...
        self.assertRedirects(
            response,
            "/helios/elections/%s/password_voter_login?%s"
            % (
                election_id,
                urlencode({"return_url": "/helios/elections/%s/view" % election_id}),
            ),
        )
    ...
    def test_election_voters_eligibility(self):
        ...
        # update eligiblity
        response = self.client.post(
            "/helios/elections/%s/voters/eligibility" % election_id,
