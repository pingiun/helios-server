...
from helios_server.helios.models import Election
...
class ElectionForm(forms.Form):
    ...
    use_voter_aliases = forms.BooleanField(
        required=False,
        initial=False,
        help_text='If selected, voter identities will be replaced with aliases, e.g. "V12", in the ballot tracking center',
    )
    # use_advanced_audit_features = forms.BooleanField(required=False, initial=True, help_text='disable this only if you want a simple election with reduced security but a simpler user interface')
    randomize_answer_order = forms.BooleanField(
        required=False,
        initial=False,
        help_text="enable this if you want the answers to questions to appear in random order for each voter",
    )
    private_p = forms.BooleanField(
        required=False,
        initial=False,
        label="Private?",
        help_text="A private election is only visible to registered voters.",
    )
    ...
    if settings.ALLOW_ELECTION_INFO_URL:
        election_info_url = forms.CharField(
            required=False,
            initial="",
            label="Election Info Download URL",
            help_text="the URL of a PDF document that contains extra election information, e.g. candidate bios and statements",
        )
...
class ElectionTimeExtensionForm(forms.Form):
    voting_extended_until = SplitDateTimeField(
        help_text="UTC date and time voting extended to",
        widget=SplitSelectDateTimeWidget,
        required=False,
    )
...
class EmailVotersForm(forms.Form):
    ...
    send_to = forms.ChoiceField(
        ...
        choices=[
            ("all", "all voters"),
            ("voted", "voters who have cast a ballot"),
            ("not-voted", "voters who have not yet cast a ballot"),
        ],
    )
...
class TallyNotificationEmailForm(forms.Form):
    ...
    send_to = forms.ChoiceField(
        ...
        choices=[
            ("all", "all voters"),
            ("voted", "only voters who cast a ballot"),
            ("none", "no one -- are you sure about this?"),
        ],
    )
