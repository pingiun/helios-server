...
class SecretKey:
    ...
    def prove_decryption(self, ciphertext):
        ...
        m = (
            inverse(pow(ciphertext.alpha, self.x, self.pk.p), self.pk.p)
            * ciphertext.beta
        ) % self.pk.p
...
class Ciphertext:
    ...
    def simulate_encryption_proof(self, plaintext, challenge=None):
        ...
        proof.commitment["B"] = (
            inverse(pow(beta_over_plaintext, proof.challenge, self.pk.p), self.pk.p)
    ...
    def decrypt(self, decryption_factors, public_key):
        ...
        for dec_factor in decryption_factors:
            running_decryption = (
                running_decryption * inverse(dec_factor, public_key.p)
            ) % public_key.p
