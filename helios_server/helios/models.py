...
from helios_server.helios_auth.jsonfield import JSONField
from helios_server.helios_auth.models import User, AUTH_SYSTEMS
...
class Election(HeliosModel):
    ...
    def last_alias_num(self):
        ...
        return self.voter_set.filter(alias_num__isnull=False).order_by("-alias_num").first()
    ...
    def add_voters_file(self, uploaded_file):
        ...
        new_voter_file = VoterFile(
            election=self, voter_file_content=uploaded_file.read().decode("utf-8")
        )
        new_voter_file.save()
    ...
    def voting_has_stopped(self):
        ...
        return (
            voting_end is not None and datetime.datetime.utcnow() >= voting_end
    ...
    def set_eligibility(self):
        ...
        voter_types = [
            ...
            if r["user__user_type"] is not None
        ]
    ...
    def url(self):
        ...
        return views.get_election_url(self)
    ...
    def itervoters(self):
        if self.voter_file_content:
            if isinstance(self.voter_file_content, str):
                ...
                raise TypeError(
                    "voter_file_content is of type {0} instead of str or bytes".format(
                        str(type(self.voter_file_content))
                    )
                )
            ...
            close = False
            voter_stream = io.StringIO(content)
        else:
            close = True
    ...
    def process(self):
        ...
        if election.use_voter_aliases:
            ...
            for i, voter in enumerate(new_voters):
                voter.alias_num = voter_alias_integers[i]
                voter.save()
...
class Voter(HeliosModel):
    ...
    alias_num = models.IntegerField(null=True)
...
class CastVote(HeliosModel):
    ...
    def set_tinyhash(self):
        ...
        """
        safe_hash = (
            self.vote_hash.decode()
            if isinstance(self.vote_hash, bytes)
            else self.vote_hash
        )
        for c in ["/", "+", "#"]:
