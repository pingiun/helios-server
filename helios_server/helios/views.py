...
import helios_server.helios_auth.url_names as helios_auth_urls
from . import utils, VOTERS_EMAIL, VOTERS_UPLOAD, url_names
from helios_server.helios_auth import views as auth_views
from helios_server.helios_auth.auth_systems import AUTH_SYSTEMS, can_list_categories
from helios_server.helios_auth.models import AuthenticationExpired
from helios_server.helios_auth.security import (
    check_csrf,
    login_required,
    get_user,
...
def user_reauth(request, user):
    ...
    login_url = "%s%s?%s" % (
        ...
        urlencode({"return_url": request.get_full_path()}),
    )
...
def election_shortcut(request, election_short_name):
    ...
    if election:
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST
            + reverse(url_names.election.ELECTION_VIEW, args=[election.uuid])
        )
    else:
        raise Http404
...
def _election_vote_shortcut(request, election):
        ...
        urlencode(
            {
                "election_url": reverse(
                    url_names.election.ELECTION_HOME, args=[election.uuid]
                )
            }
        ),
    )
    ...
    test_cookie_url = "%s?%s" % (
        reverse(url_names.COOKIE_TEST),
        urlencode({"continue_url": vote_url}),
    )
...
def test_cookie(request):
    ...
    next_url = "%s?%s" % (
        reverse(url_names.COOKIE_TEST_2),
        urlencode({"continue_url": continue_url}),
    )
...
def nocookies(request):
    retest_url = "%s?%s" % (
        reverse(url_names.COOKIE_TEST),
        urlencode({"continue_url": request.GET["continue_url"]}),
    )
...
def new_trustee(request, election):
    if request.method == "GET":
        ...
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST
            + reverse(url_names.election.ELECTION_TRUSTEES_VIEW, args=[election.uuid])
        )
...
def trustee_upload_pk(request, election, trustee):
    ...
    return HttpResponseRedirect(
        settings.SECURE_URL_HOST
        + reverse(
            url_names.election.ELECTION_TRUSTEE_HOME, args=[election.uuid, trustee.uuid]
        )
    )
...
def password_voter_login(request, election):
    ...
    if not login_url:
        ...
        if election.private_p:
            login_url = reverse(
                url_names.election.ELECTION_PASSWORD_VOTER_LOGIN, args=[election.uuid]
            )
        else:
    ...
    if password_login_form.is_valid():
        try:
            ...
            redirect_url = (
                login_url
                + "?"
                + urlencode({"bad_voter_login": "1", "return_url": return_url})
            )
...
def one_election_cast_confirm(request, election):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST
            + reverse(one_election_cast_done, args=[election.uuid])
        )
...
def one_election_copy(request, election):
    ...
    return HttpResponseRedirect(
        settings.SECURE_URL_HOST
        + reverse(url_names.election.ELECTION_VIEW, args=[new_election.uuid])
    )
...
def one_election_compute_tally(request, election):
    ...
    if not _check_election_tally_type(election):
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST
            + reverse(url_names.election.ELECTION_VIEW, args=[election.election_id])
        )
...
def voters_list_pretty(request, election):
    ...
    if q != "":
        if election.use_voter_aliases:
            voters = voters.filter(alias_num__icontains=q)
        else:
