...
from helios_server.helios_auth import models, views
    ...
    def test_unique_users(self):
        ...
        for auth_system, auth_system_module in AUTH_SYSTEMS.items():
            models.User.objects.create(
                user_type=auth_system, user_id="foobar", info={"name": "Foo Bar"}
            )

            def double_insert():
                models.User.objects.create(
                    user_type=auth_system, user_id="foobar", info={"name": "Foo2 Bar"}
                )
