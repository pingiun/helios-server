...
from helios_server.helios.utils import urlencode
from . import DEFAULT_AUTH_SYSTEM, ENABLED_AUTH_SYSTEMS
...
def index(request):
    ...
    if len(ENABLED_AUTH_SYSTEMS) == 1 and not user:
        return HttpResponseRedirect(
            reverse(AUTH_START, args=[ENABLED_AUTH_SYSTEMS[0]])
            + "?return_url="
...
def login_box_raw(request, return_url="/", auth_systems=None):
    ...
    if auth_systems is not None:
        enabled_auth_systems = (
            set(auth_systems)
            .intersection(set(ENABLED_AUTH_SYSTEMS))
...
def after(request):
    ...
    if user:
        ...
        return HttpResponseRedirect(
            ...
            % (
                reverse(AUTH_WHY),
                urlencode({"system_name": request.session["auth_system_name"]}),
            )
        )
