...
def _get_service_url():
    # FIXME current URL
    from helios_server.helios_auth import url_names
    from django.conf import settings
...
""" % (
    ...
    return {
        "user_id": values.get("mail", None),
        "name": values.get("displayName", None),
...
"""
    ...
    if len(values) > 0:
        return {
            "name": values[0].firstChild.wholeText,
            "category": values[1].firstChild.wholeText,
        }
    else:
        return None
