...
from helios_server.helios_auth import utils
...
def update_status(user_id, user_info, token, message):
    ...
    result = facebook_post(
        "/me/feed", {"access_token": token["access_token"], "message": message}
    )
...
def get_user_groups(user):
    groups_raw = utils.from_json(
        facebook_get("/me/groups", {"access_token": user.token["access_token"]})
    )
...
def pretty_eligibility(constraint):
    return (
        'Facebook users who are members of the "%s" group' % constraint["group"]["name"]
    )
