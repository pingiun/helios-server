...
import time, array, warnings, os.path
from helios.crypto.number import long_to_bytes
...
class RandomPool:
    ...
    def __init__(self, numbytes=160, cipher=None, hash=None):
        if hash is None:
            ...
            hash = (
                lambda x="": sha1(x.encode("utf-8")) if isinstance(x, str) else sha1(x)
            )
    ...
    def add_event(self, s=b""):
        ...
        """
        event = int(time.time() * 1000)
        delta = int(self._noise())
        s = s + long_to_bytes(event) + 4 * b"\xAA" + long_to_bytes(delta)
        self._addBytes(s)
    ...
    def _noise(self):
        ...
        t = time.time()
        delta = int((t - self._lastcounter) / self._ticksize * 1e6)
        self._lastcounter = t
        self._addBytes(long_to_bytes(int(1000 * time.time())))
        self._addBytes(long_to_bytes(int(1000 * time.process_time())))
        self._addBytes(long_to_bytes(int(1000 * time.time())))
        self._addBytes(long_to_bytes(delta))
