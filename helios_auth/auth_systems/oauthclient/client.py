...
class LoginOAuthClient(oauth.OAuthClient):
    ...
    def oauth_request(self, url, args=None, method=None):
        ...
        req = oauth.OAuthRequest.from_consumer_and_token(
            self.consumer, self.token, method, url, args
        )
    ...
    def get_request_token(self):
        ...
        try:
            self.token = oauth.OAuthConsumer(
    ...
    def get_access_token(self, token=None, verifier=None):
        if verifier:
            r = self.oauth_request(
                self.access_token_url(), args={"oauth_verifier": verifier}
            )
        else:
...
if __name__ == "__main__":
    ...
    response_client = LoginOAuthClient(
        consumer_key, consumer_secret, token, token_secret, server_params={}
    )
    tok = response_client.get_access_token()
    print("Making signed request")
    # verify user access
    content = response_client.oauth_request(
        "https://twitter.com/account/verify_credentials.json", method="POST"
    )
    # make an update
